...
do
	...
	-e=*|--function=*)
	endTime="${arg#*=}"
        shift # Remove --end= from processing
        ;;
	-p=*|--package=*)
	package="${arg#*=}"
...
do
    ...
    echo "$curTS | Last found: $startcpu - $lastcpu | Looking for $startTime - $endTime" >> cpu-dumpsys.txt
    if [ $endTime -ge $startcpu ]
    then
        if [ "$endTime" -le "$lastcpu" ]
        then
            cpu="$tmpCpu $(cat out.tmp | grep -m 1 $package)"
            #echo "ended $cpu"
        else
